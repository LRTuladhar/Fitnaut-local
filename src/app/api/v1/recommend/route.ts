...
function loadLibrary() {
  ...
  cachedLibrary = JSON.parse(raw).exercises.map((e: any) => ({
...
export async function POST(request: NextRequest) {
  ...
  if (!apiKey) {
    return Response.json({ ok: false, error: "API key not configured. Use Settings to add one." }, { status: 400 });
  }
  ...
  try {
    ...
    return Response.json({ ok: false, error: e.message ?? "AI request failed" }, { status: 502 });
  }
  ...
  return Response.json({ ok: true, data: { workoutSummary: result.workoutSummary, recommendations: matched } });
}
